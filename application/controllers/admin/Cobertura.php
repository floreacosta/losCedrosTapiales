...
class Cobertura extends CI_Controller {
    ...
    public function crearCobertura($nombre = null, $imagen = null) {
        if (null === $nombre) {
            ...
            $data['result'] = $this->Cobertura_model->crearCobertura($nombre, $imagen);
            $data['coberturas'] = $this->Cobertura_model->getCoberturas();
    ...
    public function updateCoberturas($id = null, $nombre = null, $imagen = null) {
        if ($imagen === null) {
            $data['result'] = $this->Cobertura_model->editarCoberturaSinImagen($id, $nombre, $imagen);
        } else {
            $data['result'] = $this->Cobertura_model->editarCobertura($id, $nombre, $imagen);
        }
    ...
    public function EliminarCobertura() {
      if ($this->input->get('id') !== null) {
            $id = $this->input->get('id');
            $data['result'] = $this->Cobertura_model->eliminarCobertura($id);
            $data['coberturas'] = $this->Cobertura_model->getCoberturas();
            $data['tipo'] = 'eliminar';
            $this->load->view('admin/includes/head');
            $this->load->view('admin/coberturas/index', $data);
