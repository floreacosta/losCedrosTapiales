...
class Especialidad extends CI_Controller {
    ...
    public function crearEspecialidad() {
        if (null === ($this->input->post('nombre'))) {
            ...
            $data['result'] = $this->Especialidad_model->crearEspecialidad($nombre_post);
            $data['especialidades'] = $this->Especialidad_model->getEspecialidades();
            $data['tipo'] = 'crear';
            $this->load->view('admin/includes/head');
            $this->load->view('admin/especialidades/index', $data);
    ...
    public function editarFormularioEspecialidades() {
        ...
        if ($this->input->get('id') !== null) {
            ...
            $data['especialidad'] = $this->Especialidad_model->getEspecialidad($id);
            $this->load->view('admin/includes/head');
    ...
    public function updateEspecialidades() {
        ...
        $nombre_post = $this->input->post('nombre');
        $data['result'] = $this->Especialidad_model->editarEspecialidad($id_post, $nombre_post);
        $data['especialidades'] = $this->Especialidad_model->getEspecialidades();
        $data['tipo'] = 'editar';
        $this->load->view('admin/includes/head');
        $this->load->view('admin/especialidades/index', $data);
    ...
    public function EliminarEspecialidad() {
      if ($this->input->get('id') !== null) {
            $id = $this->input->get('id');
            $data['result'] = $this->Especialidad_model->eliminarEspecialidad($id);
            $data['especialidades'] = $this->Especialidad_model->getEspecialidades();
            $data['tipo'] = 'eliminar';
            $this->load->view('admin/includes/head');
            $this->load->view('admin/especialidades/index', $data);
